...
import { changeFilterAC, deleteTodoTC, TodolistDomainType, updateTodoTC } from "../model/todolistsSlice"
...
export const Todolist = memo(({ todolist }: TodolistPropsType) => {
    ...
    const changeFilter = useCallback(
        (id: string, value: FilterTodolist) => {
            dispatch(changeFilterAC({ id, value }))
        },
        [dispatch],
    )
    const updateTodlistTitle = useCallback(
        (title: string) => {
    ...
    return (
        <div className="task-conteiner">
            <div>
                ...
                <button disabled={todolist.entityStatus === "loading"} onClick={removeTodolist}>
                    x
                </button>
                <h5>{dateCreate}</h5>
